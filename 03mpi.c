  ...
  for(int ht_wise=0; ht_wise<roll; ht_wise++)
    {
      ...
      if(rank!=0)
	MPI_Isend(&(imloc.pixels[0]), pvt_area+1, MPI_CHAR, 0, tag, MPI_COMM_WORLD, &request_send);
	  ...
	  for(int i=0; i<width; i++)
	    im.pixels[ht_wise*size*pvt_area+i] = imloc.pixels[i];
	      ...
	      MPI_Irecv(&(im.pixels[l]), pvt_area+1, MPI_CHAR, r, tag, MPI_COMM_WORLD, &request_recv);
	    }
	  if(size>1)
    ...
    // printf("time taken in parallel is: %f ",para_t);
